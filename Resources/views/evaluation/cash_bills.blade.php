...
?>
@extends('layouts.app')
@section('content_title','Cash Bills')
@section('content_description','View payments by cash')

@section('content')
<div class="box box-info">
    <div class="box-header">
        <h3 class="box-title">Cash Bills</h3>
    </div>
    <div class="box-body">
        @if(!$cash->isEmpty())
        <table class="table table-condensed table-responsive" id="patients">
            <tbody>
                ...
                <tr id="payment{{$payment->id}}">
                    <td>{{$payment->id}}</td>
                    <td><a href="{{route('finance.evaluation.payment_details', $payment->id)}}">{{$payment->receipt}}</a></td>
